...
public class TextMeshCreator
{
    ...
    public TextMesh createTextMesh(Text text)
    {
    ...
    private List<TextLine> createStructure(Text text)
    {
        ...
        TextWord currentWord = new TextWord(text.getFontSize());
        for(char c : text.getTextString().toCharArray())
        {
            ...
            if(ascii == SPACE_ASCII)
            {
                ...
                currentWord = new TextWord(text.getFontSize());
                continue;
            }
    ...
    private void completeStructure(List<TextLine> lines, TextLine currentLine,
                                   TextWord currentWord, Text text)
{
    boolean added = currentLine.addWord(currentWord);
    if(!added)
    {
    ...
    private TextMesh createQuadVertices(Text text,
                                        List<TextLine> lines)
    {
        ...
        for(int i = 0; i < lines.size(); i++) {
            ...
            for(TextWord word : lines.get(i).getWords())
            {
                for(TextChar letter : word.getCharacters())
                {
