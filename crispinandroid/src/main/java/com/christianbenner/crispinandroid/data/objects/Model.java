...
import com.christianbenner.crispinandroid.data.Texel;
import com.christianbenner.crispinandroid.data.Vertex;
...
public class Model {
    ...
    private ArrayList<FaceData> fArray;
    ...
    private void findInts(String line)
    {
        if(line.length() > 2)
        {
            ...
            // Add the face data
            FaceData[] faceData = new FaceData[3];
            if(foundVertex && foundTexel && foundNormal)
            {
                // Data Contains Vertices, Texels and Normals
                if(loadVertex && loadTexel && loadNormal)
                {
                    faceData[0] = new FaceData(faces.get(0), faces.get(1), faces.get(2));
                    faceData[1] = new FaceData(faces.get(3), faces.get(4), faces.get(5));
                    faceData[2] = new FaceData(faces.get(6), faces.get(7), faces.get(8));
                }
                else if(loadVertex && loadTexel)
                {
                    faceData[0] = new FaceData(faces.get(0), faces.get(1), true);
                    faceData[1] = new FaceData(faces.get(3), faces.get(4), true);
                    faceData[2] = new FaceData(faces.get(6), faces.get(7), true);
                }
                else if(loadVertex && loadNormal)
                {
                    faceData[0] = new FaceData(faces.get(0), faces.get(2), false);
                    faceData[1] = new FaceData(faces.get(3), faces.get(5), false);
                    faceData[2] = new FaceData(faces.get(6), faces.get(8), false);
                }
                else
                {
                    faceData[0] = new FaceData(faces.get(0));
                    faceData[1] = new FaceData(faces.get(3));
                    faceData[2] = new FaceData(faces.get(6));
                }
            }
            else if(foundVertex && foundTexel)
            {
                ...
                {
                    faceData[0] = new FaceData(faces.get(0), faces.get(1), true);
                    faceData[1] = new FaceData(faces.get(3), faces.get(3), true);
                    faceData[2] = new FaceData(faces.get(6), faces.get(5), true);
                }
                else
                {
                    faceData[0] = new FaceData(faces.get(0));
            ...
            else if(foundVertex && foundNormal)
            {
                ...
                {
                    faceData[0] = new FaceData(faces.get(0), faces.get(1), false);
                    faceData[1] = new FaceData(faces.get(2), faces.get(3), false);
                    faceData[2] = new FaceData(faces.get(4), faces.get(5), false);
                }
                else
                {
                    faceData[0] = new FaceData(faces.get(0));
            ...
            {
                faceData[0] = new FaceData(faces.get(0));
                faceData[1] = new FaceData(faces.get(1));
                faceData[2] = new FaceData(faces.get(2));
            }
            else
            {
                // Error
                System.out.println("The facedata cannot be processed because no vertex, normal or" +
                        " texel data exists");
            }

            // Add To Faces Array
            fArray.add(faceData[0]);
            fArray.add(faceData[1]);
            fArray.add(faceData[2]);
        }
    }
